...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            <a href="{{ route('admin.dashboard') }}" class="block text-blue-500 hover:text-blue-700">
                {{ __('Admin Dashboard') }}</a>Clearance Requests
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex bg-white overflow-scroll shadow-sm sm:rounded-lg">
                ...
                <div class="w-1/4  p-4 bg-gray-200">
                    ...
                    <!-- Sidebar links -->
                    <a href="{{ route('admin.clearance-requests') }}" class="block text-blue-500 hover:text-blue-700">
                        Clearance Requests</a>
                    <a href="{{ route('admin.clearance-history', ['status' => 'approved']) }}"
                        class="block text-blue-500 hover:text-blue-700">Approved History</a>
                    <a href="{{ route('admin.clearance-history', ['status' => 'rejected']) }}"
    ...
    text-lg mb-4">Pending Clearance Requests</h3>

@if (collect($clearanceRequests)->isEmpty())
...
@else
    <table class="table table-bordered">
        ...
        <tbody>
            @foreach ($clearanceRequests as $index => $request)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    ...
                    <td>
                        @if ($departments->has($request->user_id))
                            <a class="btn btn-info"
                                href="{{ route('admin.view-data', ['user_id' => $request->user_id, 'unit_id' => $request->unit_id]) }}">
                                View Data
                            </a>
                        @else
                            N/A
                    ...
                    <td>
                        <form method="POST" action="{{ route('admin.approve-clearance', $request->id) }}">
                            @csrf
                            <input style="background-color: green;" type="submit" class="btn btn-success"
                                value="Approve">
                        </form>
                    </td>
                    <td>
                        <a class="btn btn-danger" href="{{ route('admin.reject-clearance', $request->id) }}">Reject</a>
                    </td>
                </tr>
            @endforeach
        </tbody>
